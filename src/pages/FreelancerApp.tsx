...
const translations = {
    ...
    fr: {
        ...
        guideItems: ["Équipement de bureau à domicile", "Abonnements logiciels", "Formation professionnelle", "Déplacements pour rendez-vous clients"],
        close: "Fermer",
...
export default function FreelancerApp() {
    ...
    const handleDownloadReport = (e: React.MouseEvent) => {
        e.preventDefault();
        e.stopPropagation();
        const content = `Annual Tax Report 2026\n\nTotal Revenue: $${invoices.reduce((acc, inv) => acc + (inv.status === 'paid' ? inv.amount : 0), 0)}\nTotal Expenses: $${receipts.reduce((acc, r) => acc + r.amount, 0)}\n\nGenerated by Freelance Super-Dashboard`;
        const blob = new Blob([content], { type: 'text/plain;charset=utf-8' });
        triggerDownload(blob, 'Tax_Report_2026.txt');
    };
                        ...
                        {invoices.map(inv => (
                            <tr key={inv.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-all group">
                                ...
                                <td className="px-8 py-6 text-right space-x-2">
                                    <button onClick={() => deleteInvoice(inv.id)} title="Delete invoice" className="p-2 bg-rose-500/10 hover:bg-rose-500 rounded-lg transition-all text-rose-500 hover:text-white active:scale-90">
                                        <Trash2 size={14} />
                                    </button>
                                </td>
                            </tr>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {clients.map(client => (
                    <div key={client.id} className="bg-[#111113] border border-white/5 rounded-[32px] p-8 hover:border-blue-500/50 transition-all group relative">
                        <button title="Delete client" onClick={() => deleteClient(client.id)} className="absolute top-6 right-6 p-2 bg-white/5 hover:bg-rose-600 rounded-lg transition-all text-gray-500 hover:text-white opacity-0 group-hover:opacity-100"><Trash2 size={14} /></button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-[#111113] border border-white/5 rounded-[40px] p-8 flex flex-col items-center justify-center text-center">
                    ...
                    <button
                        ...
                        className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl font-black uppercase italic tracking-widest transition-all active:scale-95 shadow-xl"
                    >
                    ...
                    <tbody>
                        {receipts.map((r) => (
                            <tr key={r.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-colors group">
                                ...
                                <td className="px-8 py-5 text-right"><button title="Delete receipt" onClick={() => deleteReceipt(r.id)} className="p-2 bg-rose-500/10 hover:bg-rose-500 rounded-lg transition-all text-rose-500 hover:text-white active:scale-90 shadow-sm"><Trash2 size={14} /></button></td>
                            </tr>
                        ))}
                    </tbody>
                </table>
        ...
        <div className="flex flex-col h-screen bg-[#0A0A0B] text-white overflow-hidden">
            <header className="flex items-center justify-between px-6 py-4 bg-[#111113] border-b border-white/5 z-20 shadow-sm">
                <div className="flex items-center gap-4">
                    <button title={t.back} onClick={() => navigate('/')} className="p-2 hover:bg-white/5 rounded-xl transition-colors text-gray-400 hover:text-white"><ArrowLeft size={20} /></button>
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-[#111113] border border-white/10 rounded-[40px] p-8 max-w-md w-full shadow-2xl">
                            ...
                            <form onSubmit={addInvoice} className="space-y-6">
                                <div><label className="text-[10px] font-black uppercase text-gray-500 mb-2 block tracking-widest">{t.clientName}</label>
                                    <select title={t.clientName} name="client" className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-blue-500 transition-all outline-none appearance-none cursor-pointer">
                ...
                {showGuide && (
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="relative bg-[#111113] border border-white/10 rounded-[40px] p-8 max-w-lg w-full shadow-2xl">
                            <button title={t.close} onClick={() => setShowGuide(false)} className="absolute top-6 right-6 p-2 bg-white/5 hover:bg-white/10 rounded-full"><X size={20} /></button>
